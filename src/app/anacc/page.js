    ...
    <main className="min-h-screen px-6 py-12">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
        {departments.map((dpt, i) => (
          <motion.div
            key={i}
            className="p-6 bg-gray-900 rounded-xl hover:bg-purple-900 transition cursor-pointer"
            whileHover={{ scale: 1.05 }}
          >
            <h2 className="text-2xl mb-2">{dpt.name}</h2>
            <p className="mb-2">{dpt.desc}</p>
            <p className="text-sm text-gray-300"><strong>Tasks:</strong> {dpt.tasks}</p>
            <p className="text-sm mt-2 text-gray-400">Meetings: Every Tuesday, Zero Period</p>
          </motion.div>
        ))}
      </div>
    </main>
